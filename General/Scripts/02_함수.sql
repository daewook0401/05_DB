...
/* DECODE(컬럼명, 조건1, 결과1, 조건2, 결과2 ...[,마우것도 만족 X] )
 * 
 * - 컬럼 값이 일치하는 조건이 있으면
 * 	 해당 조건 오른쪽에 작성된 결과가 반환되는 함수
 * 
 * - switch, case, deault과 비슷
 * */

-- EMPLOYEE 테이블에서
-- 모든 사웜의 이름, 주민등록번호, 성별 조회

SELECT
	EMP_NAME,
	EMP_NO,
	DECODE(SUBSTR(EMP_NO,8,1) ,1, '남자', 2, '여자') -- AS성
FROM
	EMPLOYEE;

-- EMPLOYEE 테이블에서
-- 직급 코드가 'J7'인 사원의 급여를 20% 인상 
-- 직급 코드가 'J6'인 사원의 급여를 15% 인상 
-- 직급 코드가 'J5'인 사원의 급여를 10% 인상 
-- 나머지 사원은 5% 인상
...
-- 모든 사원 중 이름순으로 오른 차순 정렬 했을 때
-- 첫 번째 사원, 마지막 사원의 이름 조회
-- (문자열 대소 비교 : 유니코드 순서 A < Z, ㄱ<)
SELECT
	MIN(EMP_NAME), MAX(EMP_NAME)
FROM
	EMPLOYEE;

-- 모든 사원 중 입사일이 가장 빠른 사원, 늦은 사원의
-- 입사일 조회
SELECT
 ...
 * 
 * 2) COUNT(컬럼명)
 * - 지정된 컬럼 값이 NULL인 행을 제외하고 카운트 수행
 * 	 나머지 행의 개수 반환
 * 
...
SELECT	
--	COUNT(DECODE(SUBSTR(EMP_NO,8,1) ,1, '남자', 2, '여자')) -- AS성

...
INSERT INTO V_DCOPY2 VALUES('D2','L2');
-- 가상 테이블인데 데이터 삽입이 됟나.
-- 뷰가 아닌 원본 테이블에 데이터가 삽입된다.


SELECT * FROM V_DCOPY2;
SELECT * FROM DEPT_COPY2;

-- V_DCOP2 수정 (REPLACE 옵션 사용)
...
-- 자동으로 NULL이 삽입됨
--> LOCATION_ID는 NOT_NULL 제약조건이 설정되어 있기 때문에 NULL 삽입 불가
...
-- 인덱스를 활용해서 검색하는 방법
/* - SELECT문의 WHERE 절에 INDEX가 설정된 컬럼을 언급
 */
...
SELECT COUNT(*) FROM TB_IDX_TEST; -- ROW 100만개

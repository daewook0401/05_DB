...
/* GROUP BY가 사용된 SELECT문의 SELECT 절에는
 * 그룹 함수 또는
 * (GROUP BY에 작성된 컬럼명만 자성가능하다
 * */
...
SELECT
	...
	J.JOB_NAME,
	CASE
		-- 현재 행의 EMP_ID와 일치하는 값이 서브쿼리 결과에 있으면 '사수'
		-- 없으면 '사원'
		WHEN E.EMP_ID IN (
...
WHERE
	J.JOB_NAME = '대리'
AND
	E.SALARY > ANY ( -- 과장 최소 급여(320만)보다 많이 받는 대리 조회
		SELECT
			E.SALARY
		FROM
			EMPLOYEE E
...
SELECT
	E.EMP_ID, E.EMP_NAME, E.DEPT_CODE, J.JOB_CODE, D.DEPT_TITLE,
	J.JOB_NAME
FROM
	EMPLOYEE E
 ...
 * 
 * 1) 메인 쿼리 1줄을 해석
 * 2) 해석된 메인 쿼리 1행의 컬럼 값을 서브 쿼리에서 사용
 * 
 * 3) (메인 쿼리 WHERE절 사용 시)
...
SELECT MAIN.EMP_ID, MAIN.EMP_NAME, NVL(MAIN.MANAGER_ID, '없음'),
NVL((
SELECT SUB.EMP_NAME
FROM EMPLOYEE SUB
WHERE SUB.EMP_ID = MAIN.MANAGER_ID
...
SELECT NVL(E.DEPT_CODE, '없음') "부서 코드", 
...
WHERE --ROWNUM 이용 -> 1부터 시작하지 않아서 조회결과 X
...
SELECT * FROM EMPLOYEE;
-- 2.
SELECT MAIN.EMP_ID, MAIN.EMP_NAME, MAIN.PHONE, MAIN.SALARY, J.JOB_NAME
FROM EMPLOYEE MAIN
JOIN JOB J ON (MAIN.JOB_CODE = J.JOB_CODE)
WHERE
	MAIN.SALARY >= (
...
-- 3.
SELECT MAIN.EMP_ID, MAIN.EMP_NAME, MAIN.DEPT_CODE,
J.JOB_CODE,D.DEPT_TITLE,J.JOB_NAME
FROM EMPLOYEE MAIN
JOIN JOB J ON (MAIN.JOB_CODE = J.JOB_CODE)
JOIN DEPARTMENT D ON (MAIN.DEPT_CODE = D.DEPT_ID)
WHERE
	(MAIN.DEPT_CODE, MAIN.JOB_CODE) = (
...
-- 5.
SELECT MAIN.EMP_ID, MAIN.EMP_NAME, MAIN.DEPT_CODE,
MAIN.MANAGER_ID, MAIN.EMP_NO, MAIN.HIRE_DATE
FROM EMPLOYEE MAIN
WHERE (MAIN.DEPT_CODE, MAIN.MANAGER_ID) =
(  
	SELECT SUB.DEPT_CODE, SUB.MANAGER_ID
	FROM EMPLOYEE SUB
	WHERE SUB.EMP_NO LIKE '87%' 
	AND SUBSTR(SUB.EMP_NO,8,1)=2
)


-- 6.
SELECT MAIN.EMP_ID, MAIN.EMP_NAME, 
NVL(D.DEPT_TITLE,'소속없음'), J.JOB_NAME, MAIN.HIRE_DATE
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT D ON (MAIN.DEPT_CODE = D.DEPT_ID)
JOIN JOB J ON (MAIN.JOB_CODE = J.JOB_CODE)
WHERE MAIN.HIRE_DATE = 
...
TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE('19' || SUBSTR(MAIN.EMP_NO,1,6),'YYYYMMDD'))/12) AS "만 나이",  
TO_CHAR((MAIN.SALARY * (1 + NVL(MAIN.BONUS, 0)) * 12), 'L999,999,999') "연봉"
FROM EMPLOYEE MAIN
JOIN JOB J ON (MAIN.JOB_CODE = J.JOB_CODE)
WHERE TO_NUMBER(SUBSTR(MAIN.EMP_NO,1,6)) = 
(
	SELECT MAX(TO_NUMBER(SUBSTR(SUB.EMP_NO,1,6)))
	FROM EMPLOYEE SUB
	WHERE NVL(SUB.JOB_CODE, '소속 없음')	 = NVL(MAIN.JOB_CODE,'소속 없음')
)
ORDER BY "만 나이" DESC;

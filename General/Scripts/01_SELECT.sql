...
/* 컬럼 값 : 행과 열이 교차되는 한 칸에 삭정된 값
 * 
 * - SELECT문 작성 시
 * 	 SELECT절 컬럼명에 산술 연산을 작성하면
 *	 조회 결과(RESULT SET)에서 
 *	 모든 행에 산술 연산이 적용된 컬럼 값이 조회한다
*/

-- EMPLOYEE 테이블에서
-- 모든 사원의 이름 , 급여, 급여+100만회
...
-- EMPLOYEE 테이블에서
-- 모든 사원의 사번, 이름, 입사일, 현재까지 근무 일 수, 연차 조회
...
SELECT 
	...
	CEIL((CURRENT_DATE - HIRE_DATE) / 365) "연차 조회" -- 올림 처리
FROM EMPLOYEE;

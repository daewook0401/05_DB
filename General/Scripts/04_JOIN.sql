...
SELECT T1.JOB_CODE, T2.JOB_CODE
...
-- 21행 조회
-- 2행이 아니라 21행이 나옴
-- EMPLOYEE의 DEPT_CODE 값이 NULL인 행이 2개 있지만
-- DEPARTMENT의 DEPT_ID는 NULL 행이 없기 때문에 연결 안됨
...
-- ANSI 표준
SELECT
	EMP_NAME, DEPT_CODE, DEPT_ID, NVL(DEPT_TITLE, '부서없음') AS DEPT_TITME
FROM
...
FROM
	EMPLOYEE E
JOIN
	SAL_GRADE S ON (E.SALARY BETWEEN S.MIN_SAL AND MAX_SAL);
...
SELECT
	...
	NVL2(BONUS, (SALARY*(1+BONUS))*12, SALARY*12) 연봉
FROM
	EMPLOYEE E
JOIN
	JOB J ON(E.JOB_CODE = J.JOB_CODE)
...
FROM
	EMPLOYEE E
JOIN
	EMPLOYEE M ON (E.DEPT_CODE = M.DEPT_CODE)
WHERE

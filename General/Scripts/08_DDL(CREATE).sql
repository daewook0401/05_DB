...
INSERT INTO "MEMBER"
VALUES('MEM03','ZXCV1234','박철수','970101-1765432',DEFAULT);
...
/*
    ...
    데이터 무결성 보장을 목적으로 함.
		(데이터 무결성 : 결점 없는 데이터, 신뢰도가 높은 데이터 --> 중복X)
...
--	외부 테이블의 값을 저장할 수 있다(== 다른 테이블에서 사용할 수 있다(참조))
...
CREATE TABLE USER_USED_FK2(
  ...
  EMAIL VARCHAR2(50),
  --컬럼 레벨 설정
  GRADE_CODE NUMBER CONSTRAINT GRANDE_CODE_FK2
...
/* - CHECK 제약조건은 설정이 간편하지만 조건을 수정하는데 어려움이 있다.
 * -> 그래서 조건이 다양한 경우에는 FK 제약 조건을 이용하는 것이 좋다.
 */
...
-- 체크 제약조건(KH04_KDW.CHECK_SCORE)이 위배되었습니다
...
CREATE TABLE USER_TEST(	PNO CHAR(14) CONSTRAINT UK_PNO UNIQUE CONSTRAINT NN_PNO NOT NULL);
...
INSERT INTO USER_TEST VALUES(
	1, 'kh1', 'kh1', '123456-1234567', '남', '010-9191-1919','서울 종로구','N'
);
INSERT INTO USER_TEST VALUES(
	2, 'kh2', 'kh2', '654321-2654321', '여', '010-1234-1234','서울 종로구','N'
);
INSERT INTO USER_TEST VALUES(
	3, 'kh3', 'kh3', '990101-1958423', '남', '010-1212-1212','서울 종로구','Y'
);
INSERT INTO USER_TEST VALUES(
	4, 'kh4', 'kh4', '950505-225196', '여', '010-8888-7777','서울 종로구','Y'
);
INSERT INTO USER_TEST VALUES(
	5, 'kh5', 'kh5', '801230-1234567', '남', '010-4555-2323','서울 종로구','N'
);
----------------------------------------------------------------------------------------------------------------
...
--> 서브쿼리로 테이블 생성 시
-- 별도의 제약조건 설정, DEFAULT 설정, COMMENT 설정 등이 필요함
...
CREATE TABLE EMPLOYEE_COPY2 AS 
	SELECT E.EMP_ID, E.EMP_NAME, NVL(D.DEPT_TITLE, '없음') AS DEPT_TITLE,
				 J.JOB_NAME, E.SALARY
	FROM EMPLOYEE E
 ...
 * 
 * NOT NULL은 컬럼의 상태(NULL 허용, NULL 불가)를 수정(MODIFY)하는 모습
 */
...
ALTER TABLE EMPLOYEE_COPY ADD CONSTRAINT DEPT_CODE_ID FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT(DEPT_ID);


-- EMPLOYEE테이블의 DEPT_CODE에 외래키 제약조건 추가
-- 참조 테이블은 DEPARTMENT, 참조 컬럼은 DEPARTMENT의 기본키
ALTER TABLE EMPLOYEE ADD CONSTRAINT DEPT_CODE_ID FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT(DEPT_ID);

-- EMPLOYEE테이블의 JOB_CODE 외래키 제약조건 추가
-- 참조 테이블은 JOB, 참조 컬럼은 JOB의 기본키
ALTER TABLE EMPLOYEE ADD CONSTRAINT JOB_CODE_ID FOREIGN KEY(JOB_CODE) REFERENCES JOB(JOB_CODE);
...
ALTER TABLE LOCATION ADD CONSTRAINT NATIONAL_CODE_FK FOREIGN KEY(NATIONAL_CODE) REFERENCES NATIONAL(NATIONAL_CODE);
